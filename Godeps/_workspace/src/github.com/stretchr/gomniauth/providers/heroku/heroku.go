...
const (
	herokuDefaultScope    string = "global"
	herokuName            string = "heroku"
	herokuDisplayName     string = "heroku"
	herokuAuthURL         string = "https://id.heroku.com/oauth/authorize"
	herokuTokenURL        string = "https://id.heroku.com/oauth/token"
	herokuEndpointProfile string = "https://api.heroku.com/account"
)

// herokuProvider implements the Provider interface and provides heroku
...
func New(clientId, clientSecret, redirectUrl string) *HerokuProvider {

	p := new(HerokuProvider)
	p.config = &common.Config{Map: objx.MSI(
		oauth2.OAuth2KeyAuthURL, herokuAuthURL,
		oauth2.OAuth2KeyTokenURL, herokuTokenURL,
		oauth2.OAuth2KeyClientID, clientId,
		oauth2.OAuth2KeySecret, clientSecret,
		oauth2.OAuth2KeyRedirectUrl, redirectUrl,
		oauth2.OAuth2KeyScope, herokuDefaultScope,
...
func (provider *HerokuProvider) TripperFactory() common.TripperFactory {
...
func (provider *HerokuProvider) PublicData(options map[string]interface{}) (interface{}, error) {
	return gomniauth.ProviderPublicData(provider, options)
}

// Name is the unique name for this provider.
func (provider *HerokuProvider) Name() string {
	return herokuName
}

// DisplayName is the human readable name for this provider.
func (provider *HerokuProvider) DisplayName() string {
	return herokuDisplayName
}
...
func (provider *HerokuProvider) GetBeginAuthURL(state *common.State, options objx.Map) (string, error) {
	if options != nil {
		scope := oauth2.MergeScopes(options.Get(oauth2.OAuth2KeyScope).Str(), herokuDefaultScope)
...
func (provider *HerokuProvider) Get(creds *common.Credentials, endpoint string) (objx.Map, error) {
...
func (provider *HerokuProvider) GetUser(creds *common.Credentials) (common.User, error) {

	profileData, err := provider.Get(creds, herokuEndpointProfile)
...
func (provider *HerokuProvider) CompleteAuth(data objx.Map) (*common.Credentials, error) {
...
// protected heroku resources
func (provider *HerokuProvider) GetClient(creds *common.Credentials) (*http.Client, error) {
